...
class GetOrganizationExportView(APIView):
    """
    Get details on an org export request
    Matches Asana API: GET /organization_exports/{organization_export_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        responses={
            200: OpenApiResponse(
                response=GetOrganizationExportResponseSerializer,
    ...
    def get(self, request, organization_export_gid: str):
        # Validate organization_export_gid format
        try:
            validate_uuid(organization_export_gid)
        except Exception:
            return Response(
                invalid_gid_error("organization_export_gid"),
