...
from asana_project_briefs.interactors.update_project_brief_interactor import (
    UpdateProjectBriefInteractor
)
from asana_project_briefs.storages.storage_implementation import (
    StorageImplementation
)
from asana_project_briefs.presenters.update_project_brief_presenter_implementation import (
    UpdateProjectBriefPresenterImplementation
)
...
class UpdateProjectBriefView(APIView):
    """
    Update a project brief
    Matches Asana API: PUT /project_briefs/{project_brief_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=UpdateProjectBriefRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=UpdateProjectBriefResponseSerializer,
    ...
    def put(self, request, project_brief_gid: str):
        ...
        presenter = UpdateProjectBriefPresenterImplementation()
        interactor = UpdateProjectBriefInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = UpdateProjectBriefRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"project briefs: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.update_project_brief(
