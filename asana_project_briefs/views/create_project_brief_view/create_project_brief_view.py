...
from asana_project_briefs.interactors.create_project_brief_interactor import (
    CreateProjectBriefInteractor
)
from asana_project_briefs.storages.storage_implementation import (
    StorageImplementation
)
from asana_project_briefs.presenters.create_project_brief_presenter_implementation import (
    CreateProjectBriefPresenterImplementation
)
...
class CreateProjectBriefView(APIView):
    """
    Create a project brief
    Matches Asana API: POST /projects/{project_gid}/project_briefs
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=CreateProjectBriefRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=CreateProjectBriefResponseSerializer,
    ...
    def post(self, request, project_gid: str):
        ...
        presenter = CreateProjectBriefPresenterImplementation()
        interactor = CreateProjectBriefInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateProjectBriefRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.create_project_brief(
