...
from asana_goals.interactors.remove_custom_field_setting_for_goal_interactor import (
    RemoveCustomFieldSettingForGoalInteractor
)
from asana_goals.storages.storage_implementation import (
    StorageImplementation
)
from asana_goals.presenters.remove_custom_field_setting_for_goal_presenter_implementation import (
    RemoveCustomFieldSettingForGoalPresenterImplementation
)
...
class RemoveCustomFieldSettingForGoalView(APIView):
    """
    Remove a custom field from a goal
    Matches Asana API: POST /goals/{goal_gid}/removeCustomFieldSetting
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveCustomFieldSettingForGoalRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveCustomFieldSettingForGoalResponseSerializer,
    ...
    def post(self, request, goal_gid: str):
        ...
        presenter = RemoveCustomFieldSettingForGoalPresenterImplementation()
        interactor = RemoveCustomFieldSettingForGoalInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = RemoveCustomFieldSettingForGoalRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"goals: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.remove_custom_field_setting_for_goal(
                goal_gid=goal_gid,
