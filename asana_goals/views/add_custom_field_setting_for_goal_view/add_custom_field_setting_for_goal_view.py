...
from asana_goals.interactors.add_custom_field_setting_for_goal_interactor import (
    AddCustomFieldSettingForGoalInteractor
)
from asana_goals.storages.storage_implementation import (
    StorageImplementation
)
from asana_goals.presenters.add_custom_field_setting_for_goal_presenter_implementation import (
    AddCustomFieldSettingForGoalPresenterImplementation
)
...
class AddCustomFieldSettingForGoalView(APIView):
    """
    Add a custom field to a goal
    Matches Asana API: POST /goals/{goal_gid}/addCustomFieldSetting
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=AddCustomFieldSettingForGoalRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=AddCustomFieldSettingForGoalResponseSerializer,
    ...
    def post(self, request, goal_gid: str):
        ...
        presenter = AddCustomFieldSettingForGoalPresenterImplementation()
        interactor = AddCustomFieldSettingForGoalInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = AddCustomFieldSettingForGoalRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.add_custom_field_setting_for_goal(
                goal_gid=goal_gid,
