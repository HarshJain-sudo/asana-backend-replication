...
from asana_goals.interactors.remove_followers_interactor import (
    RemoveFollowersInteractor
)
from asana_goals.storages.storage_implementation import (
    StorageImplementation
)
from asana_goals.presenters.remove_followers_presenter_implementation import (
    RemoveFollowersPresenterImplementation
)
...
class RemoveFollowersView(APIView):
    """
    Remove a collaborator from a goal
    Matches Asana API: POST /goals/{goal_gid}/removeFollowers
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveFollowersRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveFollowersResponseSerializer,
    ...
    def post(self, request, goal_gid: str):
        ...
        presenter = RemoveFollowersPresenterImplementation()
        interactor = RemoveFollowersInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = RemoveFollowersRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.remove_followers(
                goal_gid=goal_gid,
