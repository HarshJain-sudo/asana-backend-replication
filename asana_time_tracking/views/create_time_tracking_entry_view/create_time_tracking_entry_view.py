...
from asana_time_tracking.interactors.create_time_tracking_entry_interactor import (
    CreateTimeTrackingEntryInteractor
)
from asana_time_tracking.storages.storage_implementation import (
    StorageImplementation
)
from asana_time_tracking.presenters.create_time_tracking_entry_presenter_implementation import (
    CreateTimeTrackingEntryPresenterImplementation
)
...
class CreateTimeTrackingEntryView(APIView):
    """
    Create a time tracking entry
    Matches Asana API: POST /tasks/{task_gid}/time_tracking_entries
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=CreateTimeTrackingEntryRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=CreateTimeTrackingEntryResponseSerializer,
    ...
    def post(self, request, task_gid: str):
        ...
        presenter = CreateTimeTrackingEntryPresenterImplementation()
        interactor = CreateTimeTrackingEntryInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateTimeTrackingEntryRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.create_time_tracking_entry(
                task_gid=task_gid,
