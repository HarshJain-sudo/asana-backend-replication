...
from asana_projects.interactors.create_project_for_team_interactor import (
    CreateProjectForTeamInteractor
)
from asana_projects.storages.storage_implementation import (
    StorageImplementation
)
from asana_projects.presenters.create_project_for_team_presenter_implementation import (
    CreateProjectForTeamPresenterImplementation
)
...
class CreateProjectForTeamView(APIView):
    """
    Create a project in a team
    Matches Asana API: POST /teams/{team_gid}/projects
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=CreateProjectForTeamRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=CreateProjectForTeamResponseSerializer,
    ...
    def post(self, request, team_gid: str):
        ...
        presenter = CreateProjectForTeamPresenterImplementation()
        interactor = CreateProjectForTeamInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateProjectForTeamRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.create_project_for_team(
                team_gid=team_gid,
