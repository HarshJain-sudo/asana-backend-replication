...
from asana_projects.interactors.create_project_for_workspace_interactor import (
    CreateProjectForWorkspaceInteractor
)
from asana_projects.storages.storage_implementation import (
    StorageImplementation
)
from asana_projects.presenters.create_project_for_workspace_presenter_implementation import (
    CreateProjectForWorkspacePresenterImplementation
)
...
class CreateProjectForWorkspaceView(APIView):
    """
    Create a project in a workspace
    Matches Asana API: POST /workspaces/{workspace_gid}/projects
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=CreateProjectForWorkspaceRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=CreateProjectForWorkspaceResponseSerializer,
    ...
    def post(self, request, workspace_gid: str):
        ...
        presenter = CreateProjectForWorkspacePresenterImplementation()
        interactor = CreateProjectForWorkspaceInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateProjectForWorkspaceRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.create_project_for_workspace(
