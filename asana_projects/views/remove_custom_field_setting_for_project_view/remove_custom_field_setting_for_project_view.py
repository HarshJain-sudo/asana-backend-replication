...
from asana_projects.interactors.remove_custom_field_setting_for_project_interactor import (
    RemoveCustomFieldSettingForProjectInteractor
)
from asana_projects.storages.storage_implementation import (
    StorageImplementation
)
from asana_projects.presenters.remove_custom_field_setting_for_project_presenter_implementation import (
    RemoveCustomFieldSettingForProjectPresenterImplementation
)
...
class RemoveCustomFieldSettingForProjectView(APIView):
    """
    Remove a custom field from a project
    Matches Asana API: POST /projects/{project_gid}/removeCustomFieldSetting
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveCustomFieldSettingForProjectRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveCustomFieldSettingForProjectResponseSerializer,
    ...
    def post(self, request, project_gid: str):
        ...
        presenter = RemoveCustomFieldSettingForProjectPresenterImplementation()
        interactor = RemoveCustomFieldSettingForProjectInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = RemoveCustomFieldSettingForProjectRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.remove_custom_field_setting_for_project(
