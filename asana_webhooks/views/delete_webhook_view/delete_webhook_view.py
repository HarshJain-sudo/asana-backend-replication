...
from asana_webhooks.interactors.delete_webhook_interactor import (
    DeleteWebhookInteractor
)
from asana_webhooks.storages.storage_implementation import (
    StorageImplementation
)
from asana_webhooks.presenters.delete_webhook_presenter_implementation import (
    DeleteWebhookPresenterImplementation
)
...
class DeleteWebhookView(APIView):
    """
    Delete a webhook
    Matches Asana API: DELETE /webhooks/{webhook_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        responses={
            200: OpenApiResponse(
                response=DeleteWebhookResponseSerializer,
    ...
    def delete(self, request, webhook_gid: str):
        # Validate webhook_gid format
        try:
            validate_uuid(webhook_gid)
        except Exception:
            return Response(
                invalid_gid_error("webhook_gid"),
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Initialize dependencies
        storage = StorageImplementation()
        presenter = DeleteWebhookPresenterImplementation()
        interactor = DeleteWebhookInteractor(
            storage=storage,
            presenter=presenter
        )
        ...
        try:
            ...
            response = interactor.delete_webhook(
