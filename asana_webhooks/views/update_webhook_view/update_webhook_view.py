...
from asana_webhooks.interactors.update_webhook_interactor import (
    UpdateWebhookInteractor
)
from asana_webhooks.storages.storage_implementation import (
    StorageImplementation
)
from asana_webhooks.presenters.update_webhook_presenter_implementation import (
    UpdateWebhookPresenterImplementation
)
...
class UpdateWebhookView(APIView):
    """
    Update a webhook
    Matches Asana API: PUT /webhooks/{webhook_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        responses={
            200: OpenApiResponse(
                response=UpdateWebhookResponseSerializer,
    ...
    def put(self, request, webhook_gid: str):
        ...
        presenter = UpdateWebhookPresenterImplementation()
        interactor = UpdateWebhookInteractor(
            storage=storage,
            presenter=presenter
        )
        ...
        try:
            ...
            response = interactor.update_webhook(
