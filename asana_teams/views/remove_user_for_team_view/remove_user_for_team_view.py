...
from asana_teams.interactors.remove_user_for_team_interactor import (
    RemoveUserForTeamInteractor
)
from asana_teams.storages.storage_implementation import (
    StorageImplementation
)
from asana_teams.presenters.remove_user_for_team_presenter_implementation import (
    RemoveUserForTeamPresenterImplementation
)
...
class RemoveUserForTeamView(APIView):
    """
    Remove a user from a team
    Matches Asana API: POST /teams/{team_gid}/removeUser
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveUserForTeamRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveUserForTeamResponseSerializer,
    ...
    def post(self, request, team_gid: str):
        ...
        presenter = RemoveUserForTeamPresenterImplementation()
        interactor = RemoveUserForTeamInteractor(
            storage=storage,
            presenter=presenter
        )
        ...
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"teams: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.remove_user_for_team(
                team_gid=team_gid,
