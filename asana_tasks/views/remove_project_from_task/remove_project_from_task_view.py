...
from asana_tasks.interactors.remove_project_from_task_interactor import (
    RemoveProjectFromTaskInteractor
)
...
class RemoveProjectFromTaskView(APIView):
    ...
    def post(self, request, task_gid: str):
        ...
        interactor = RemoveProjectFromTaskInteractor(
            storage=storage,
            presenter=presenter
        )

        try:
            response = interactor.remove_project_from_task(
                task_gid,
