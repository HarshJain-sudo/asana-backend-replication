...
from asana_tasks.interactors.add_dependencies_for_task_interactor import (
    AddDependenciesForTaskInteractor
)
from asana_tasks.storages.storage_implementation import (
    StorageImplementation
)
from asana_tasks.presenters.add_dependencies_for_task_presenter_implementation import (
    AddDependenciesForTaskPresenterImplementation
)
...
class AddDependenciesForTaskView(APIView):
    """
    Set dependencies for a task
    Matches Asana API: POST /tasks/{task_gid}/addDependencies
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=AddDependenciesForTaskRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=AddDependenciesForTaskResponseSerializer,
    ...
    def post(self, request, task_gid: str):
        ...
        presenter = AddDependenciesForTaskPresenterImplementation()
        interactor = AddDependenciesForTaskInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = AddDependenciesForTaskRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.add_dependencies_for_task(
                task_gid=task_gid,
