...
from asana_tasks.interactors.remove_dependents_for_task_interactor import (
    RemoveDependentsForTaskInteractor
)
from asana_tasks.storages.storage_implementation import (
    StorageImplementation
)
from asana_tasks.presenters.remove_dependents_for_task_presenter_implementation import (
    RemoveDependentsForTaskPresenterImplementation
)
...
class RemoveDependentsForTaskView(APIView):
    """
    Unlink dependents from a task
    Matches Asana API: POST /tasks/{task_gid}/removeDependents
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveDependentsForTaskRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveDependentsForTaskResponseSerializer,
    ...
    def post(self, request, task_gid: str):
        ...
        presenter = RemoveDependentsForTaskPresenterImplementation()
        interactor = RemoveDependentsForTaskInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = RemoveDependentsForTaskRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.remove_dependents_for_task(
                task_gid=task_gid,
