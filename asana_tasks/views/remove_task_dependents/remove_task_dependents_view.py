...
from asana_tasks.interactors.remove_task_dependents_interactor import RemoveTaskDependentsInteractor
...
class RemoveTaskDependentsView(APIView):
    ...
    @extend_schema(
                    ...
                    'data': {
                        ...
                        'properties': {
                            'dependents': {
                                'type': 'array',
                                'items': {'type': 'string'},
                                'description': 'List of task GIDs to remove as dependents'
                            }
                        }
                    }
                }
        ...
        responses={
            200: OpenApiResponse(
                ...
                description="Task dependents removed successfully."
            ),
            400: OpenApiResponse(
        ...
        summary="Remove task dependents",
        description="Removes specific dependents from a task.",
        tags=["Tasks"]
    )
    def post(self, request, task_gid: str):
        ...
        interactor = RemoveTaskDependentsInteractor(
            storage=storage,
            presenter=presenter
        )

        try:
            response = interactor.remove_task_dependents(task_gid, dependent_gids)
