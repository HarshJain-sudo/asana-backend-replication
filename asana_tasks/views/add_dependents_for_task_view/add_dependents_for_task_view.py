...
from asana_tasks.interactors.add_dependents_for_task_interactor import (
    AddDependentsForTaskInteractor
)
from asana_tasks.storages.storage_implementation import (
    StorageImplementation
)
from asana_tasks.presenters.add_dependents_for_task_presenter_implementation import (
    AddDependentsForTaskPresenterImplementation
)
...
class AddDependentsForTaskView(APIView):
    """
    Set dependents for a task
    Matches Asana API: POST /tasks/{task_gid}/addDependents
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=AddDependentsForTaskRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=AddDependentsForTaskResponseSerializer,
    ...
    def post(self, request, task_gid: str):
        ...
        presenter = AddDependentsForTaskPresenterImplementation()
        interactor = AddDependentsForTaskInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = AddDependentsForTaskRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.add_dependents_for_task(
                task_gid=task_gid,
