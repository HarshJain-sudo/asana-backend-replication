...
class GetTasksForUserTaskListView(APIView):
    """
    Get tasks from a user task list
    Matches Asana API: GET /user_task_lists/{user_task_list_gid}/tasks
    """
...
*Note: You can only pass in an offset that was returned to you via a previously paginated request.*',
        ...
        responses={
            200: OpenApiResponse(
                response=GetTasksForUserTaskListResponseSerializer,
    ...
    def get(self, request, user_task_list_gid: str):
        # Validate user_task_list_gid format
        try:
            validate_uuid(user_task_list_gid)
        except Exception:
            return Response(
                invalid_gid_error("user_task_list_gid"),
