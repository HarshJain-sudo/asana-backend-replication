...
from asana_tasks.interactors.remove_task_dependencies_interactor import RemoveTaskDependenciesInteractor
...
class RemoveTaskDependenciesView(APIView):
    ...
    @extend_schema(
                    ...
                    'data': {
                        ...
                        'properties': {
                            'dependencies': {
                                'type': 'array',
                                'items': {'type': 'string'},
                                'description': 'List of task GIDs to remove as dependencies'
                            }
                        }
                    }
                }
        ...
        responses={
            200: OpenApiResponse(
                ...
                description="Task dependencies removed successfully."
            ),
            400: OpenApiResponse(
        ...
        description="Removes specific dependencies from a task.",
        tags=["Tasks"]
    )
    def post(self, request, task_gid: str):
        ...
        interactor = RemoveTaskDependenciesInteractor(
            storage=storage,
            presenter=presenter
        )

        try:
            response = interactor.remove_task_dependencies(task_gid, dependency_gids)
