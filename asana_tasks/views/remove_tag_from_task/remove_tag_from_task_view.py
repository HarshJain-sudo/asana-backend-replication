...
from asana_tasks.interactors.remove_tag_from_task_interactor import (
    RemoveTagFromTaskInteractor
)
...
class RemoveTagFromTaskView(APIView):
    ...
    def post(self, request, task_gid: str):
        ...
        interactor = RemoveTagFromTaskInteractor(
            storage=storage,
            presenter=presenter
        )

        try:
            response = interactor.remove_tag_from_task(
                task_gid,
