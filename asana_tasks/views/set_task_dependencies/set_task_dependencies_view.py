...
from asana_tasks.interactors.set_task_dependencies_interactor import SetTaskDependenciesInteractor
...
class SetTaskDependenciesView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                ...
                description="Task dependencies set successfully."
            ),
            400: OpenApiResponse(
                response=ErrorResponseSerializer,
                description="Bad Request - Invalid task GID format or dependency GIDs."
            ),
            404: OpenApiResponse(
                response=ErrorResponseSerializer,
                description="Not Found - Task or dependency task does not exist."
            ),
        },
        summary="Set task dependencies",
        description="Sets the tasks that this task depends on.",
        tags=["Tasks"]
    )
    def post(self, request, task_gid: str):
        ...
        interactor = SetTaskDependenciesInteractor(
            storage=storage,
            presenter=presenter
        )

        try:
            response = interactor.set_task_dependencies(task_gid, dependency_gids)
