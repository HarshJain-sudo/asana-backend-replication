...
from asana_tasks.interactors.remove_dependencies_for_task_interactor import (
    RemoveDependenciesForTaskInteractor
)
from asana_tasks.storages.storage_implementation import (
    StorageImplementation
)
from asana_tasks.presenters.remove_dependencies_for_task_presenter_implementation import (
    RemoveDependenciesForTaskPresenterImplementation
)
...
class RemoveDependenciesForTaskView(APIView):
    """
    Unlink dependencies from a task
    Matches Asana API: POST /tasks/{task_gid}/removeDependencies
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveDependenciesForTaskRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveDependenciesForTaskResponseSerializer,
    ...
    def post(self, request, task_gid: str):
        ...
        presenter = RemoveDependenciesForTaskPresenterImplementation()
        interactor = RemoveDependenciesForTaskInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = RemoveDependenciesForTaskRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"tasks: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.remove_dependencies_for_task(
                task_gid=task_gid,
