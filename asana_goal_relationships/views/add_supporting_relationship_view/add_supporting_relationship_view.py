...
from asana_goal_relationships.interactors.add_supporting_relationship_interactor import (
    AddSupportingRelationshipInteractor
)
from asana_goal_relationships.storages.storage_implementation import (
    StorageImplementation
)
from asana_goal_relationships.presenters.add_supporting_relationship_presenter_implementation import (
    AddSupportingRelationshipPresenterImplementation
)
...
class AddSupportingRelationshipView(APIView):
    """
    Add a supporting goal relationship
    Matches Asana API: POST /goals/{goal_gid}/addSupportingRelationship
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=AddSupportingRelationshipRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=AddSupportingRelationshipResponseSerializer,
    ...
    def post(self, request, goal_gid: str):
        ...
        presenter = AddSupportingRelationshipPresenterImplementation()
        interactor = AddSupportingRelationshipInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = AddSupportingRelationshipRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.add_supporting_relationship(
                goal_gid=goal_gid,
