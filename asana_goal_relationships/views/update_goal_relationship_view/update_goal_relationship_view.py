...
from asana_goal_relationships.interactors.update_goal_relationship_interactor import (
    UpdateGoalRelationshipInteractor
)
from asana_goal_relationships.storages.storage_implementation import (
    StorageImplementation
)
from asana_goal_relationships.presenters.update_goal_relationship_presenter_implementation import (
    UpdateGoalRelationshipPresenterImplementation
)
...
class UpdateGoalRelationshipView(APIView):
    """
    Update a goal relationship
    Matches Asana API: PUT /goal_relationships/{goal_relationship_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=UpdateGoalRelationshipRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=UpdateGoalRelationshipResponseSerializer,
    ...
    def put(self, request, goal_relationship_gid: str):
        # Validate goal_relationship_gid format
        try:
            validate_uuid(goal_relationship_gid)
        except Exception:
            return Response(
                invalid_gid_error("goal_relationship_gid"),
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Initialize dependencies
        storage = StorageImplementation()
        presenter = UpdateGoalRelationshipPresenterImplementation()
        interactor = UpdateGoalRelationshipInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = UpdateGoalRelationshipRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.update_goal_relationship(
