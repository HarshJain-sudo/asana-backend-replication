...
from asana_goal_relationships.interactors.remove_supporting_relationship_interactor import (
    RemoveSupportingRelationshipInteractor
)
from asana_goal_relationships.storages.storage_implementation import (
    StorageImplementation
)
from asana_goal_relationships.presenters.remove_supporting_relationship_presenter_implementation import (
    RemoveSupportingRelationshipPresenterImplementation
)
...
class RemoveSupportingRelationshipView(APIView):
    """
    Removes a supporting goal relationship
    Matches Asana API: POST /goals/{goal_gid}/removeSupportingRelationship
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveSupportingRelationshipRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveSupportingRelationshipResponseSerializer,
    ...
    def post(self, request, goal_gid: str):
        # Validate goal_gid format
        try:
            validate_uuid(goal_gid)
        except Exception:
            return Response(
                invalid_gid_error("goal_gid"),
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Initialize dependencies
        storage = StorageImplementation()
        presenter = RemoveSupportingRelationshipPresenterImplementation()
        interactor = RemoveSupportingRelationshipInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = RemoveSupportingRelationshipRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"goal relationships: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.remove_supporting_relationship(
                goal_gid=goal_gid,
