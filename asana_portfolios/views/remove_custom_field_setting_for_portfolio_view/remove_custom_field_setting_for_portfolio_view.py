...
from asana_portfolios.interactors.remove_custom_field_setting_for_portfolio_interactor import (
    RemoveCustomFieldSettingForPortfolioInteractor
)
from asana_portfolios.storages.storage_implementation import (
    StorageImplementation
)
from asana_portfolios.presenters.remove_custom_field_setting_for_portfolio_presenter_implementation import (
    RemoveCustomFieldSettingForPortfolioPresenterImplementation
)
...
class RemoveCustomFieldSettingForPortfolioView(APIView):
    """
    Remove a custom field from a portfolio
    Matches Asana API: POST /portfolios/{portfolio_gid}/removeCustomFieldSetting
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveCustomFieldSettingForPortfolioRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveCustomFieldSettingForPortfolioResponseSerializer,
    ...
    def post(self, request, portfolio_gid: str):
        ...
        presenter = RemoveCustomFieldSettingForPortfolioPresenterImplementation()
        interactor = RemoveCustomFieldSettingForPortfolioInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = RemoveCustomFieldSettingForPortfolioRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.remove_custom_field_setting_for_portfolio(
