...
from asana_portfolios.interactors.add_custom_field_setting_for_portfolio_interactor import (
    AddCustomFieldSettingForPortfolioInteractor
)
from asana_portfolios.storages.storage_implementation import (
    StorageImplementation
)
from asana_portfolios.presenters.add_custom_field_setting_for_portfolio_presenter_implementation import (
    AddCustomFieldSettingForPortfolioPresenterImplementation
)
...
class AddCustomFieldSettingForPortfolioView(APIView):
    """
    Add a custom field to a portfolio
    Matches Asana API: POST /portfolios/{portfolio_gid}/addCustomFieldSetting
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=AddCustomFieldSettingForPortfolioRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=AddCustomFieldSettingForPortfolioResponseSerializer,
    ...
    def post(self, request, portfolio_gid: str):
        ...
        presenter = AddCustomFieldSettingForPortfolioPresenterImplementation()
        interactor = AddCustomFieldSettingForPortfolioInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = AddCustomFieldSettingForPortfolioRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.add_custom_field_setting_for_portfolio(
