...
from asana_portfolios.interactors.add_members_for_portfolio_interactor import (
    AddMembersForPortfolioInteractor
)
from asana_portfolios.storages.storage_implementation import (
    StorageImplementation
)
from asana_portfolios.presenters.add_members_for_portfolio_presenter_implementation import (
    AddMembersForPortfolioPresenterImplementation
)
...
class AddMembersForPortfolioView(APIView):
    """
    Add users to a portfolio
    Matches Asana API: POST /portfolios/{portfolio_gid}/addMembers
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=AddMembersForPortfolioRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=AddMembersForPortfolioResponseSerializer,
    ...
    def post(self, request, portfolio_gid: str):
        ...
        presenter = AddMembersForPortfolioPresenterImplementation()
        interactor = AddMembersForPortfolioInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = AddMembersForPortfolioRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.add_members_for_portfolio(
