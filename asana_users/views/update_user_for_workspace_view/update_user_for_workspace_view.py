...
from asana_users.interactors.update_user_for_workspace_interactor import (
    UpdateUserForWorkspaceInteractor
)
from asana_users.storages.storage_implementation import (
    StorageImplementation
)
from asana_users.presenters.update_user_for_workspace_presenter_implementation import (
    UpdateUserForWorkspacePresenterImplementation
)
...
class UpdateUserForWorkspaceView(APIView):
    """
    Update a user in a workspace or organization
    Matches Asana API: PUT /workspaces/{workspace_gid}/users/{user_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=UpdateUserForWorkspaceRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=UpdateUserForWorkspaceResponseSerializer,
    ...
    def put(self, request, workspace_gid: str, user_gid: str):
        ...
        presenter = UpdateUserForWorkspacePresenterImplementation()
        interactor = UpdateUserForWorkspaceInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = UpdateUserForWorkspaceRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.update_user_for_workspace(
