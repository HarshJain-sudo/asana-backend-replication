...
from asana_workspaces.interactors.remove_user_for_workspace_interactor import (
    RemoveUserForWorkspaceInteractor
)
from asana_workspaces.storages.storage_implementation import (
    StorageImplementation
)
from asana_workspaces.presenters.remove_user_for_workspace_presenter_implementation import (
    RemoveUserForWorkspacePresenterImplementation
)
...
class RemoveUserForWorkspaceView(APIView):
    """
    Remove a user from a workspace or organization
    Matches Asana API: POST /workspaces/{workspace_gid}/removeUser
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=RemoveUserForWorkspaceRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=RemoveUserForWorkspaceResponseSerializer,
    ...
    def post(self, request, workspace_gid: str):
        ...
        presenter = RemoveUserForWorkspacePresenterImplementation()
        interactor = RemoveUserForWorkspaceInteractor(
            storage=storage,
            presenter=presenter
        )
        ...
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"workspaces: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.remove_user_for_workspace(
