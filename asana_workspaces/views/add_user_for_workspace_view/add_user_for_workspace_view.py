...
from asana_workspaces.interactors.add_user_for_workspace_interactor import (
    AddUserForWorkspaceInteractor
)
from asana_workspaces.storages.storage_implementation import (
    StorageImplementation
)
from asana_workspaces.presenters.add_user_for_workspace_presenter_implementation import (
    AddUserForWorkspacePresenterImplementation
)
...
class AddUserForWorkspaceView(APIView):
    """
    Add a user to a workspace or organization
    Matches Asana API: POST /workspaces/{workspace_gid}/addUser
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=AddUserForWorkspaceRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=AddUserForWorkspaceResponseSerializer,
    ...
    def post(self, request, workspace_gid: str):
        ...
        presenter = AddUserForWorkspacePresenterImplementation()
        interactor = AddUserForWorkspaceInteractor(
            storage=storage,
            presenter=presenter
        )
        ...
        try:
            ...
            response = interactor.add_user_for_workspace(
