...
from asana_status_updates.interactors.create_status_for_object_interactor import (
    CreateStatusForObjectInteractor
)
from asana_status_updates.storages.storage_implementation import (
    StorageImplementation
)
from asana_status_updates.presenters.create_status_for_object_presenter_implementation import (
    CreateStatusForObjectPresenterImplementation
)
...
class CreateStatusForObjectView(APIView):
    """
    Create a status update
    Matches Asana API: POST /status_updates
    """
...
*Note: You can only pass in an offset that was returned to you via a previously paginated request.*',
        ...
        request=CreateStatusForObjectRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=CreateStatusForObjectResponseSerializer,
    ...
    def post(self, request):
        ...
        presenter = CreateStatusForObjectPresenterImplementation()
        interactor = CreateStatusForObjectInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateStatusForObjectRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"status updates: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.create_status_for_object(
