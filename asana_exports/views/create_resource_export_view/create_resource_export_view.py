...
from asana_exports.interactors.create_resource_export_interactor import (
    CreateResourceExportInteractor
)
from asana_exports.storages.storage_implementation import (
    StorageImplementation
)
from asana_exports.presenters.create_resource_export_presenter_implementation import (
    CreateResourceExportPresenterImplementation
)
...
class CreateResourceExportView(APIView):
    """
    Initiate a resource export
    Matches Asana API: POST /exports/resource
    """
    ...
    @extend_schema(
        ...
        request=CreateResourceExportRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=CreateResourceExportResponseSerializer,
    ...
    def post(self, request):
        ...
        presenter = CreateResourceExportPresenterImplementation()
        interactor = CreateResourceExportInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateResourceExportRequestSerializer(data=request.data)
        ...
        try:
            ...
            response = interactor.create_resource_export(
