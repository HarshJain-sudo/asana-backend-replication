...
from asana_tags.presenters.update_tag_presenter_implementation import (
    UpdateTagPresenterImplementation
)
...
class UpdateTagView(APIView):
    """
    Update a tag
    Matches Asana API: PUT /tags/{tag_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=UpdateTagRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=UpdateTagResponseSerializer,
    ...
    def put(self, request, tag_gid: str):
        # Validate tag_gid format
        try:
            validate_uuid(tag_gid)
        except Exception:
            return Response(
                invalid_gid_error("tag_gid"),
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Initialize dependencies
        storage = StorageImplementation()
        presenter = UpdateTagPresenterImplementation()
        ...
        serializer = UpdateTagRequestSerializer(data=request.data)
