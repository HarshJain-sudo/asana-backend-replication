...
from asana_task_templates.interactors.instantiate_task_interactor import (
    InstantiateTaskInteractor
)
from asana_task_templates.storages.storage_implementation import (
    StorageImplementation
)
from asana_task_templates.presenters.instantiate_task_presenter_implementation import (
    InstantiateTaskPresenterImplementation
)
...
class InstantiateTaskView(APIView):
    """
    Instantiate a task from a task template
    Matches Asana API: POST /task_templates/{task_template_gid}/instantiateTask
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=InstantiateTaskRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=InstantiateTaskResponseSerializer,
    ...
    def post(self, request, task_template_gid: str):
        # Validate task_template_gid format
        try:
            validate_uuid(task_template_gid)
        except Exception:
            return Response(
                invalid_gid_error("task_template_gid"),
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Initialize dependencies
        storage = StorageImplementation()
        presenter = InstantiateTaskPresenterImplementation()
        interactor = InstantiateTaskInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = InstantiateTaskRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"task templates: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.instantiate_task(
