...
from asana_stories.interactors.update_story_interactor import (
    UpdateStoryInteractor
)
from asana_stories.storages.storage_implementation import (
    StorageImplementation
)
from asana_stories.presenters.update_story_presenter_implementation import (
    UpdateStoryPresenterImplementation
)
...
class UpdateStoryView(APIView):
    """
    Update a story
    Matches Asana API: PUT /stories/{story_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=UpdateStoryRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=UpdateStoryResponseSerializer,
    ...
    def put(self, request, story_gid: str):
        # Validate story_gid format
        try:
            validate_uuid(story_gid)
        except Exception:
            return Response(
                invalid_gid_error("story_gid"),
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Initialize dependencies
        storage = StorageImplementation()
        presenter = UpdateStoryPresenterImplementation()
        interactor = UpdateStoryInteractor(
            storage=storage,
            presenter=presenter
        )
        ...
        try:
            ...
            response = interactor.update_story(
                story_gid=story_gid,
