...
from asana_stories.interactors.create_story_for_task_interactor import (
    CreateStoryForTaskInteractor
)
from asana_stories.storages.storage_implementation import (
    StorageImplementation
)
from asana_stories.presenters.create_story_for_task_presenter_implementation import (
    CreateStoryForTaskPresenterImplementation
)
...
class CreateStoryForTaskView(APIView):
    """
    Create a story on a task
    Matches Asana API: POST /tasks/{task_gid}/stories
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=CreateStoryForTaskRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=CreateStoryForTaskResponseSerializer,
    ...
    def post(self, request, task_gid: str):
        ...
        presenter = CreateStoryForTaskPresenterImplementation()
        interactor = CreateStoryForTaskInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateStoryForTaskRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"stories: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.create_story_for_task(
                task_gid=task_gid,
