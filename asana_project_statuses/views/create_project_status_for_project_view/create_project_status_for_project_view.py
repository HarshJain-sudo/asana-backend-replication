...
from asana_project_statuses.interactors.create_project_status_for_project_interactor import (
    CreateProjectStatusForProjectInteractor
)
from asana_project_statuses.storages.storage_implementation import (
    StorageImplementation
)
from asana_project_statuses.presenters.create_project_status_for_project_presenter_implementation import (
    CreateProjectStatusForProjectPresenterImplementation
)
...
class CreateProjectStatusForProjectView(APIView):
    """
    Create a project status
    Matches Asana API: POST /projects/{project_gid}/project_statuses
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=CreateProjectStatusForProjectRequestSerializer,
        responses={
            201: OpenApiResponse(
                response=CreateProjectStatusForProjectResponseSerializer,
    ...
    def post(self, request, project_gid: str):
        ...
        presenter = CreateProjectStatusForProjectPresenterImplementation()
        interactor = CreateProjectStatusForProjectInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateProjectStatusForProjectRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"project statuses: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.create_project_status_for_project(
