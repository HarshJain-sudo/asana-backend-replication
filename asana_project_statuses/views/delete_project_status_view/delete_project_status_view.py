...
from asana_project_statuses.interactors.delete_project_status_interactor import (
    DeleteProjectStatusInteractor
)
from asana_project_statuses.storages.storage_implementation import (
    StorageImplementation
)
from asana_project_statuses.presenters.delete_project_status_presenter_implementation import (
    DeleteProjectStatusPresenterImplementation
)
...
class DeleteProjectStatusView(APIView):
    """
    Delete a project status
    Matches Asana API: DELETE /project_statuses/{project_status_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        responses={
            200: OpenApiResponse(
                response=DeleteProjectStatusResponseSerializer,
    ...
    def delete(self, request, project_status_gid: str):
        # Validate project_status_gid format
        try:
            validate_uuid(project_status_gid)
        except Exception:
            return Response(
                invalid_gid_error("project_status_gid"),
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Initialize dependencies
        storage = StorageImplementation()
        presenter = DeleteProjectStatusPresenterImplementation()
        interactor = DeleteProjectStatusInteractor(
            storage=storage,
            presenter=presenter
        )
        ...
        try:
            ...
            response = interactor.delete_project_status(
