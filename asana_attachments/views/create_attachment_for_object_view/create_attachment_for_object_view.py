...
from asana_attachments.interactors.create_attachment_for_object_interactor import (
    CreateAttachmentForObjectInteractor
)
from asana_attachments.storages.storage_implementation import (
    StorageImplementation
)
from asana_attachments.presenters.create_attachment_for_object_presenter_implementation import (
    CreateAttachmentForObjectPresenterImplementation
)
...
class CreateAttachmentForObjectView(APIView):
    """
    Upload an attachment
    Matches Asana API: POST /attachments
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        request=CreateAttachmentForObjectRequestSerializer,
        responses={
            200: OpenApiResponse(
                response=CreateAttachmentForObjectResponseSerializer,
    ...
    def post(self, request):
        ...
        presenter = CreateAttachmentForObjectPresenterImplementation()
        interactor = CreateAttachmentForObjectInteractor(
            storage=storage,
            presenter=presenter
        )
        
        # Validate request data
        serializer = CreateAttachmentForObjectRequestSerializer(data=request.data)
        if not serializer.is_valid():
            error_response = {
                'errors': [{
                    'message': f"attachments: {str(serializer.errors)}",
                    'help': (
        ...
        try:
            ...
            response = interactor.create_attachment_for_object(
