...
class GetTimePeriodView(APIView):
    """
    Get a time period
    Matches Asana API: GET /time_periods/{time_period_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        responses={
            200: OpenApiResponse(
                response=GetTimePeriodResponseSerializer,
    ...
    def get(self, request, time_period_gid: str):
        # Validate time_period_gid format
        try:
            validate_uuid(time_period_gid)
        except Exception:
            return Response(
                invalid_gid_error("time_period_gid"),
