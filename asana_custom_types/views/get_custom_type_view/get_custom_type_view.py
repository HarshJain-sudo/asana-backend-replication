...
class GetCustomTypeView(APIView):
    """
    Get a custom type
    Matches Asana API: GET /custom_types/{custom_type_gid}
    """
...
Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.',
        ...
        responses={
            200: OpenApiResponse(
                response=GetCustomTypeResponseSerializer,
    ...
    def get(self, request, custom_type_gid: str):
        # Validate custom_type_gid format
        try:
            validate_uuid(custom_type_gid)
        except Exception:
            return Response(
                invalid_gid_error("custom_type_gid"),
